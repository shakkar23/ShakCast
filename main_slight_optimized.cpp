...
static void recurse(Board& board, Path& path, BitBoard& bboard, TrieNode* node, const int swaps, std::vector<Path>& largest_word) {
    ...
    for (const auto& [x1, y1] : neighbors | std::views::take(n_neighbors)) {
        ...
        if (swaps > 0) {
            for (size_t i = 0; i < N; ++i) {
                ...
                recurse(board, path, bboard, next_node, swaps - 1, largest_word);
                path.pop_back();
                bboard = bb_copy;
            }
        }
        ...
        if (node->bitfield & (1 << reserved_index)) {
            ...
            recurse(board, path, bboard, next_node, swaps, largest_word);
            path.pop_back();
            bboard = bb_copy;
        }
    }
            ...
            if (swaps > 0)
                for (int a = 'a'; a <= 'z'; ++a) {
                    ...
                    if (a != board[i][j].first)
                        swaps_left--;
                    recurse(board, path, bboard, root.get()->children.at(char_to_index(a)).get(), swaps_left, biggest_words);
                }
            else {
